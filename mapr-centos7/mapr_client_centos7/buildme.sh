...
for REPO in core mep ; do
  ...
  if containerRunning=$(docker inspect --format '{{ .State.Running }}' mapr-${REPO}-repo) ; then 
    if [[ "$containerRunning" = "true" ]] ; then
      ...
      if [[ "$repoPort" = "$hostPort" ]]; then
        :
      else
        echo "Invalid port $repoPort for mapr-${REPO}-repo. Removing container."
...
# Do not set version for mep.  launch-cluster.sh will set this if the template had not been modified by a child container
